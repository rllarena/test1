<?php
function compara_Intervalos($linea, $nemo, $hEvento,$hnex,$Dif){
  ...
  $ArrLsup = [];
    ...
    for ($i=0; $i<sizeof($id); $i++){
      ...
      if($tEvento >= $Linf and $tEvento < $Lsup){
        ...
        $timeEvento = explode(':', $Dif);
        $decTimeEvento = ($timeEvento[0]*60) + ($timeEvento[1]) + ($timeEvento[2]/60);
        $timeArr = explode(':', $IntTab);
        $decTimeTabla = ($timeArr[0]*60) + ($timeArr[1]) + ($timeArr[2]/60);
        $cadena=strval($decTimeEvento). ":" .strval($decTimeTabla*0.5). ":" .strval($decTimeTabla*1.5);
        return($cadena);
